    ...
    <div class="content-wrapper">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">Tin tức công nghệ</a></li>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <a href="{{route('tin-tuc.create')}}" class="btn btn-primary pr-2">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    @foreach($tintuc as $item)
                        <tr>
                            <td>{{$item->id}}</td>
                            <td><a href="{{route('tin-tuc.show',$item->slug)}}">{{$item->tieude}}</a></td>
                            <td>{{$item->slug}}</td>
                            <td>{{$item->trangthai?'Bật':'Tắt'}}</td>
                            <td>{{$item->updated_at}}</td>
                            <td>{{$item->tacgia}}</td>
                            <td class="d-flex">
                                <form action="{{route('tin-tuc.edit',$item->id)}}" method="get" class="pr-1">
                                    <button class="btn btn-md btn-primary rounded-5">
                                        Sửa
                                    </button>
                                </form>
                                <form action="{{route('tin-tuc.destroy',$item->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
